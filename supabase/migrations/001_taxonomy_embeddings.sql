-- =============================================================================
-- Shopify Taxonomy Embeddings Setup Script
-- =============================================================================
--
-- This script sets up the pgvector extension, table, index, and function
-- for semantic search over Shopify product taxonomy categories.
--
...
CREATE TABLE IF NOT EXISTS shopify_taxonomy_embeddings (
...
ON shopify_taxonomy_embeddings
USING ivfflat (embedding vector_cosine_ops)
WITH (lists = 100);

-- Create additional indexes for filtering
CREATE INDEX IF NOT EXISTS idx_taxonomy_level
ON shopify_taxonomy_embeddings (level);

CREATE INDEX IF NOT EXISTS idx_taxonomy_category_code
ON shopify_taxonomy_embeddings (category_code);
...
--   SELECT * FROM match_taxonomy_categories(
...
CREATE OR REPLACE FUNCTION match_taxonomy_categories(
...
BEGIN
    ...
    FROM shopify_taxonomy_embeddings t
    WHERE
...
COMMENT ON FUNCTION match_taxonomy_categories IS
...
--
-- Check total count:
--   SELECT COUNT(*) FROM shopify_taxonomy_embeddings;
--   -- Expected: 11,764
--
-- Check level distribution:
--   SELECT level, COUNT(*) FROM shopify_taxonomy_embeddings GROUP BY level ORDER BY level;
--
-- Test similarity search (requires an embedding):
--   SELECT * FROM match_taxonomy_categories(
--     (SELECT embedding FROM shopify_taxonomy_embeddings WHERE category_code = 'fb-1-7-1'),
--     0.5, 5, NULL
--   );
-- =============================================================================
